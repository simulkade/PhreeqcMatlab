...
- **Downloads are SHA-256 verified** against the release `SHA256SUMS.txt` (fetched once per release URL and cached). A mismatch deletes the file and skips the version stamp, so the next `startup` retries; a missing checksum file (releases before 3.8.6) or no digest backend warns and installs anyway. Locally installed libraries are deliberately *not* verified — a source build is a different binary from the released one and would never match. Hashing goes through `src/Tools/sha256_file.m` (JVM `MessageDigest`, falling back to `sha256sum`/`certutil`).
...
- `src/FVTool/` — couples reactive transport to the external [FVTool](https://github.com/FiniteVolumeTransportPhenomena/FVTool) finite-volume package for multi-D transport. `PhreeqcFVToolTransport` is the 2D driver (operator splitting); `InitializePhreeqcFVTool` sets initial/boundary conditions. FVTool is auto-provisioned by `startup.m` (cloned into `external/FVTool`, gitignored, and initialized via `FVToolStartUp` with the cwd saved/restored); `fvtool_available` guards it and the driver errors helpfully if provisioning failed.
